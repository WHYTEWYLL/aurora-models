...
tx_count_block_range AS (
    SELECT
        block_number,
        tx_count as block_tx_count
    FROM
        {{ ref('silver__blocks') }}
    WHERE
...
broken_blocks AS (
    ...
    SELECT BLOCK_NUMBER FROM (SELECT
        block_number
        COUNT (1) AS tx_count
    FROM
        {{ ref("silver__transactions") }} b
    GROUP BY 1
        LEFT JOIN tx_per_block
        t USING (
            block_number
        )
    )
    WHERE
        t.blocks_tx_count != b.tx_count
),
impacted_blocks AS (
    SELECT
